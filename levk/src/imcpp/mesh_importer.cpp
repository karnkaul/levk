...
void MeshImporter::setup_entries(AssetList const& asset_list, std::string_view const directory) {
	m_entries.clear();
	auto add = [directory](auto const& src, auto& dst, bool is_skinned) {
		for (auto const [asset, index] : enumerate(src)) {
			auto& entry = dst.emplace_back();
			entry.asset = asset;
			entry.display_name = fmt::format("[{}] {}", index, entry.asset.gltf_name);
			auto export_uri = entry.asset.gltf_name;
			if (!export_uri.empty()) { export_uri += "/"; }
			entry.export_uri.set(fmt::format("{}/{}", directory, export_uri));
			entry.is_skinned = is_skinned;
		}
	};
	add(asset_list.static_meshes, m_entries, false);
	add(asset_list.skinned_meshes, m_entries, true);
}

Uri MeshImporter::update() {
	if (!m_importer || m_entries.empty()) { return {}; }
	if (auto m = Modal{"Mesh Importer", {true}, {true}, ImGuiWindowFlags_AlwaysAutoResize}) {
		...
		ImGui::Text("%s", FixedString{"Type: {}", selected.is_skinned ? "SkinnedMesh" : "StaticMesh"}.c_str());
		ImGui::Separator();
