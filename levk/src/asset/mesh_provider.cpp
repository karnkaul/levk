...
StaticMeshProvider::Payload StaticMeshProvider::load_payload(Uri<StaticMesh> const& uri, Stopwatch const& stopwatch) const {
	...
	for (auto const& in_primitive : asset.primitives) {
		...
		if (!bin_geometry.read(read_bytes(in_primitive.geometry.value()).span())) {
			m_logger.error("Failed to read bin geometry [{}]", in_primitive.geometry.value());
			continue;
		}
	...
	m_logger.info("[{:.3f}s] StaticMesh loaded [{}]", stopwatch().count(), uri.value());
	return ret;
}
...
SkinnedMeshProvider::SkinnedMeshProvider(NotNull<SkeletonProvider*> skeleton_provider, NotNull<MaterialProvider*> material_provider)
	: MeshProviderCommon<SkinnedMesh>(material_provider, "SkinnedMeshProvider"), m_skeleton_provider(skeleton_provider) {}
...
SkinnedMeshProvider::Payload SkinnedMeshProvider::load_payload(Uri<SkinnedMesh> const& uri, Stopwatch const& stopwatch) const {
	...
	ret.dependencies.push_back(uri);
	m_logger.info("[{:.3f}s] SkinnedMesh loaded [{}]", stopwatch().count(), uri.value());
	return ret;
}
} // namespace levk
