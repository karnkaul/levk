...
Ptr<SkinnedMesh const> Resources::load(Uri<SkinnedMesh> const& uri) { return load<SkinnedMesh>(uri, "SkinnedMesh", &AssetLoader::load_skinned_mesh); }
...
Ptr<T const> Resources::load(Uri<> const& uri, std::string_view const type, F func) {
	auto loader = AssetLoader{*m_reader, Service<GraphicsDevice>::locate(), render};
	Ptr<T const> ret = (loader.*func)(uri);
	if (!ret) {
